...
type
  ...
  *)
  TOutputType = (otNone, otEntireLine, otBeginningOfLine);
  // modified end
  ...
  *)
  TCharDecoding = function(ASender: TObject; ABuf: RawByteString): string of object;
  // modified end
...
type
  ...
  *)
  ...
  strict private
    ...
    function get_Value: RawByteString;
    procedure set_Value(const AValue: RawByteString);
  public
    procedure Add(const AValue: RawByteString);
    procedure Delete(APos, ACount: Integer);
    function Length: Integer;
    property Value: RawByteString read get_Value write set_Value;
  end;
  // modified end
  ...
  strict protected
    ...
    *)
    function DoCharDecoding(ASender: TObject; ABuf: RawByteString): string; virtual;
    // modified end
  ...
  public
    ...
    *)
    procedure SendLine(const AValue: string; AEol, AEof: Boolean);
    // modified end
...
*)
function TDosThread.DoCharDecoding(ASender: TObject; ABuf: RawByteString): string;
// modified end
begin
...
var
  ...
  iLength: Integer;
*)
function TDosCommand.DoCharDecoding(ASender: TObject; ABuf: RawByteString): string;
// modified end
begin
...
{$IF CompilerVersion > 22.9}
    pBytes := TEncoding.ANSI.GetBytes(AChars);
{$ELSE}
    pBytes := TEncoding.Default.GetBytes(AChars);
{$IFEND}
    // modified end
...
*)
procedure TDosCommand.SendLine(const AValue: string; AEol, AEof: Boolean);
// modified end
const
...
begin
  if (FThread <> nil) then
    ...
    *)
    FThread.InputLines.Add(EolCh[AEol] + EolCh[AEof] + AValue)
    // modified end
  else
...
*)
procedure TSyncString.Add(const AValue: RawByteString);
// modified end
begin
  BeginWrite;
  try
...
*)
function TSyncString.get_Value: RawByteString;
// modified end
begin
  BeginRead;
  try
...
*)
procedure TSyncString.set_Value(const AValue: RawByteString);
// modified end
begin
  BeginWrite;
  try
